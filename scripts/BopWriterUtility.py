...
class _BopWriterUtility:
    ...
    def get_frame_gt(dataset_objects: List[bpy.types.Mesh], unit_scaling: float, ignore_dist_thres: float,
        ...
        for obj in dataset_objects:
            ...
            if obj.get_cp("category_id") < 0:
                # ######Better to remove these before render() in render_recorded_animations.py######
                # ignore all objects that are not part of the dataset                
                # print(f"BopWriterUtility: ignored obj {obj.get_name()} with category_id {obj.get_cp('category_id')} because it is not part of the dataset")
                continue
    ...
    def write_frames(chunks_dir: str, dataset_objects: list, depths: Optional[List[np.ndarray]] = None,
        ...
        frame_offset = bpy.context.scene.frame_start
        ...
        for frame_id in range(bpy.context.scene.frame_start, bpy.context.scene.frame_end):
            ...
            if colors:
                color_rgb = colors[frame_id - bpy.context.scene.frame_start]
            ...
            if depths:
                depth = depths[frame_id - bpy.context.scene.frame_start]
            else:
            ...
            if ((curr_frame_id + 1) % frames_per_chunk == 0) or \
                    (frame_id == frame_offset + num_new_frames - 1):
