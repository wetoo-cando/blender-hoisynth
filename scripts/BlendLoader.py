...
def load_blend(path: str, obj_types: Optional[Union[List[str], str]] = None, ignore_obj_by_name: Optional[Union[List[str], str]] = None, 
    ...
    for data_block in data_blocks:
        ...
        if data_block == "objects":            
            for obj in getattr(data_to, data_block):
                print(f"BlendLoader.py::load_blend(): Object {obj.name} ")
                ignore_object = False
                # Ignore objects by name
                if ignore_obj_by_name is not None:
                    for obj_name_substr in ignore_obj_by_name:
                        if obj_name_substr in obj.name:
                            ignore_object = True
                else:
                    print(f"BlendLoader.py::load_blend(): not ignoring any objects by name")
                if ignore_object is True:
                    print(f"BlendLoader.py::load_blend(): ignoring object {obj.name}")
                    continue
                if ignore_object is False and obj.hide_render is True:
                    print(f"BlendLoader.py::load_blend(): unhiding object {obj.name} for rendering")
            ...
            print("Selected " + str(len(loaded_objects)) + " of the loaded objects")
        else:
